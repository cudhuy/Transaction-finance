...
const fetchTxChallenge = async (pubKey) => {
	const txChallenge = await fetch(
		`${AUTH_API_BASE_URL}/auth/tx-challenge?pubkey=${pubKey}`,
		{
			method: 'GET',
			headers: {
...
const verifyTxChallenge = async (tx_challenge_verifier, tx_signature) => {
	...
	const verifiedTxChallenge = await fetch(
		`${AUTH_API_BASE_URL}/auth/tx-challenge`,
		{
			method: 'post',
			headers: {
...
const sendPriorityTransaction = async (jwt, tx) => {
...
const buildDemoTx = (user, toPubkey, recentBlockhash) =>
...
const run = async () => {
	...
	limitter.on(Event.TASK_FINISHED, () => {
		...
		process.stderr.write(
			`Finished requests: ${totalRequestsFinished}, Total time: ${duration}, TPS: ${
				totalRequestsFinished / duration
			}\n`,
		);
	});

	for await (const signedTx of genSignedDemoTxs(
		user,
		toPubkey,
		recentBlockhash,
	)) {
		...
		limitter.emit(
			Event.TASK_REQUEST,
			sendPriorityTransaction(authToken, signedTx),
		);
	}
	...
	console.log(
		SPAMMER_SOURCE_WALLET.publicKey.toBase58(),
		'->',
		SPAMMER_TARGET_PUBKEY.toBase58(),
	);
};
...
const fund = async (cluster) => {
	...
	const transferSum =
		sumNaturalNumbers(STARTING_LAMPORTS + TX_COUNT - 1) -
		sumNaturalNumbers(STARTING_LAMPORTS - 1);
	...
	await web3.sendAndConfirmTransaction(cluster, tx, [USER_WALLET]);
};
